...
const methods = {
    ...
    toast: function (player, data) {
        player.tm.showToast(data.type || "info", data.msg);
    },

    initUser: function (player, data) {
        // Standalone: just set the user id and go to main page
    ...
    initPlayer: function (player, data) {
        ...
        player.isDJ = true; // Always DJ in standalone
        ...
        player.repeat = data?.repeatMode || "off";
        player.channelName = data?.channelName || "Web Player";
        player.autoplay = data?.autoplay || false;
        player.updateBar(player.volumeBar, data?.volume || 100, data?.volume || 100);
...
class Player {
    constructor() {
        ...
        this.userId = null;
        this.isDJ = true; // Always DJ in standalone
        this.date = new Date();
        this.queue = [];
        ...
        window.onYouTubeIframeAPIReady = () => {
            this.ytPlayer = new YT.Player('yt-player', {
                ...
                events: {
                    ...
                    'onError': (event) => this.onYTError(event)
                }
            });
        };
        $("#queue-overlay").on("click", () => {
            closeAllModals();
        });
    }
    ...
    onYTError(event) {
        ...
        // Error codes: 2 (invalid param), 5 (HTML5 error), 100 (not found/removed), 101/150 (not allowed in embedded)
        if ([100, 101, 150].includes(event.data)) {
            this.tm.showToast("error", "This song is unavailable. Skipping...");
        }
        this.send({ op: "trackFinished" });
    }

    onYTStateChange(event) {
        console.log("YT Player State Change:", event.data);
        if (event.data === YT.PlayerState.ENDED) {
            console.log("YT Player Ended naturally. Sending trackFinished.");
            this.send({ op: "trackFinished" });
        }
    }

    syncYT() {
        if (!this.ytReady || !this.currentTrack) return;
        
        const videoId = this.currentTrack.identifier;
        const currentVideoUrl = this.ytPlayer.getVideoUrl();
        
        if (!currentVideoUrl.includes(videoId)) {
            this.ytPlayer.loadVideoById(videoId, this.currentPosition / 1000);
            if (this.isPaused) this.ytPlayer.pauseVideo();
        }
        ...
        const ytPos = this.ytPlayer.getCurrentTime() * 1000;
        if (Math.abs(ytPos - this.currentPosition) > 2000) {
            this.ytPlayer.seekTo(this.currentPosition / 1000, true);
        }
    ...
    updateMediaSession() {
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
            ...
            artist: this.currentTrack.author,
            artwork: [
                { src: this.currentTrack.artworkUrl, sizes: '512x512', type: 'image/jpg' }
            ]
        });

        navigator.mediaSession.setActionHandler('play', () => {
            this.isPaused = false;
            this.send({ op: "updatePause", pause: false });
            this.updatePlayerView();
        });
        navigator.mediaSession.setActionHandler('pause', () => {
            this.isPaused = true;
            this.send({ op: "updatePause", pause: true });
            this.updatePlayerView();
        });
