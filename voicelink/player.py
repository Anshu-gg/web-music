...
async def connect_channel(ctx: Union[commands.Context, Interaction], channel: VoiceChannel = None):
    ...
    if player.volume != 100:
        try:
            ...
            pass  # Volume will be applied when the player session is fully established
...
class Player(VoiceProtocol):
    ...
    async def _dispatch_event(self, data: dict):
        ...
        if event_cls is None:
            self._logger.debug(f"Player in {self.guild.name}({self.guild.id}) received unknown event type: {event_type}, skipping.")
            return
        ...
        if isinstance(event, TrackStartEvent):
            ...
            self._bot.loop.create_task(self._fetch_recommendations())

        self._logger.debug(f"Player in {self.guild.name}({self.guild.id}) dispatched event {event_type}.")

    async def do_next(self, _retry_count: int = 0):
        ...
        # Wait for Lavalink to confirm voice connection is established
        if not self._is_connected:
            for _ in range(20):  # Wait up to 10 seconds
                ...
                self._logger.warning(f"Player in {self.guild.name}({self.guild.id}) Lavalink not connected after 10s, proceeding anyway.")
        ...
        if not track:
            ...
            try:
                ...
                if _retry_count < 3:
                    # Re-insert the track at the front so it's not lost on retry
                    self.queue.put_at_front(track)
                    self._current = None
                    await asyncio.sleep(5)
                    return await self.do_next(_retry_count=_retry_count + 1)
                else:
                    self._logger.error(f"Giving up after {_retry_count} retries in {self.guild.name}({self.guild.id})")
                    self._current = None
                    await self.handle_queue_end()
    ...
    async def is_position_fresh(self):
        ...
        try:
            async for message in self.context.channel.history(limit=1):
    ...
    async def handle_queue_end(self):
        """Deletes the controller and sends the 'Queue Ended' message."""
        if self.controller:
            try:
                await self.controller.delete()
            except:
        ...
        embed = discord.Embed(
            title="🎵 Queue ended! Thanks for listening!",
            description="Use `/play <song>` to start playing again.\n\n"
                        "💡 **Did you know?** You can save the current song to your DMs with `,grab` — never lose a banger!",
            color=self.settings.get("embed_color", 0xb3b3b3)
        )
        
        view = discord.ui.View()
        view.add_item(discord.ui.Button(label="Support Server", url="https://discord.gg/titli", style=discord.ButtonStyle.link))
        view.add_item(discord.ui.Button(label="Vote", url="https://top.gg/bot/1488277481248260366/vote", style=discord.ButtonStyle.link))
        view.add_item(discord.ui.Button(label="Invite Me", url=f"https://discord.com/oauth2/authorize?client_id={self.bot.user.id}&permissions=2184260928&scope=bot%20applications.commands", style=discord.ButtonStyle.link))
    ...
    async def get_tracks(
        ...
        *,
        ...
        """
        if not search_type:
            platform = self.settings.get("search_platform", Config().search_platform)
            search_type = platform if isinstance(platform, SearchType) else SearchType.from_platform(str(platform))
    ...
    async def _fetch_recommendations(self) -> None:
        """Fetches similar tracks in the background and stores them for the dropdown."""
        try:
            await asyncio.sleep(15)  # Wait 15 seconds before fetching
            ...
            if tracks:
                # Store up to 10 recommendations (Discord select menu limit is 25)
                self._recommendations = tracks[:10]
                self._logger.debug(f"Player in {self.guild.name}({self.guild.id}) fetched {len(self._recommendations)} recommendations.")
                # Refresh the controller to show the recommendations in dropdown
                await self.invoke_controller()
            else:
                self._recommendations = []
        except Exception as e:
            self._logger.debug(f"Failed to fetch recommendations: {e}")
            self._recommendations = []
    
    async def get_recommendations(self, *, track: Optional[Track] = None) -> bool:
        """Fetches and adds a recommended track based on the provided track or recent history."""
        if not track:
            try:
        ...
        tracks = await track.get_recommendations(self._node)
        if tracks:
            # Only add the first recommendation to ensure disabling works as expected
            await self.add_track(tracks[0], duplicate=False)
